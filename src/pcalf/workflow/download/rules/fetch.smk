localrules: dm_ncbi_accession_from_ncbi_cli_taxid ,dm_ncbi_get_children_taxid, dm_ncbi_taxid_to_accession, dm_ncbi_get_reports
...
rule dm_ncbi:
    ...
    input:
        os.path.join(RESDIR,"datas",'accession.txt'),
    params:
...
def get_accession(wildcards):
    ...
    if config["taxid"]:
        files.append( os.path.join( RESDIR,"datas","accession.taxid.txt" ))
        files.append( os.path.join( RESDIR,"datas","accession.report.txt"))
        files.append( os.path.join( RESDIR,"datas","accession.cli.txt") )
    return files 
...
rule dm_ncbi_aggregate_accession_from_ncbi_cli_taxid:
    output:
        os.path.join(RESDIR,"datas","accession.cli.txt"),
    input:
        expand(os.path.join(RESDIR,"datas","taxids", "taxid_{taxid}", "tmp",'accession_from_taxid_cmdline.tsv'),
            taxid=config["taxid"]),
    shell:
        "cat {input} > {output} "
...
rule dm_ncbi_accession_from_ncbi_cli_taxid:
    output:
        os.path.join(RESDIR,"datas","taxids", "taxid_{taxid}", "tmp",'accession_from_taxid_cmdline.tsv'),
    conda:
...
rule dm_ncbi_filter_reports:
    ...
    '''
    output:
        os.path.join(RESDIR,"datas",'accession.report.txt'),
    input:
        reports = os.path.join(RESDIR,"reports",'{}.report.tsv'.format(date.today().strftime('%Y-%m'))),
        taxids  = os.path.join(RESDIR,"datas","taxid.txt"),     
    conda:
...
rule dm_ncbi_aggregate_taxid_accessions:
    ...
    output:
        os.path.join(RESDIR,"datas","accession.taxid.txt"),
    input:
        expand(os.path.join(RESDIR,"datas",'accession.taxid.{section}.txt'),
...
rule dm_ncbi_taxid_to_accession:
    ...
    '''
    output:
        os.path.join(RESDIR,"datas",'accession.taxid.{section}.txt'),
    input:
        os.path.join(RESDIR,"datas","taxid.txt"),
    params:
        group = config['group'],
    conda:
        "../envs/ngd.yaml"
...
rule dm_ncbi_expand_taxid:
    output:
        os.path.join(RESDIR,"datas","taxid.txt"),
    input:
        expand(os.path.join(RESDIR,"datas","taxids", "taxid_{taxid}", "tmp",'taxid.txt'),
            taxid=config["taxid"]),
    shell:
...
rule dm_ncbi_get_children_taxid:
    ...
    '''
    output:
        taxid = os.path.join(RESDIR,"datas","taxids", "taxid_{taxid}", "tmp",'taxid.txt'),
        taxidname = os.path.join(RESDIR,"datas","taxids", "taxid_{taxid}", "tmp",'taxid.name.txt'),
    log:
...
rule dm_ncbi_get_reports:
    ...
    '''
    output:
        os.path.join(RESDIR,"reports",'{}.report.tsv'.format(date.today().strftime('%Y-%m'))),
    log:
        os.path.join(RESDIR,"reports",'report.log'),  
    conda:
        "../envs/biopython_1.79.yaml"
    script:
