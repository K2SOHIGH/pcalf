...
rule cm_empty_gtdbtk_summary:
    ...
    run:
        import pandas as pd
        pd.DataFrame(
            columns = GTDBSUMMARYCOLS,
            index = list(params.genomes.keys())            
            ).to_csv(str(output),sep="\t",header=True)
                
rule cm_target_quick_gtdbtk:
    output:
        touch(os.path.join(RESDIR,"gtdbtk-res","gtdbtk.quick.done"))
    input:
...
rule gm_target_gtdbtk:
    output:
        touch(os.path.join(RESDIR,"gtdbtk-res","gtdbtk.done"))
    input:
        os.path.join(RESDIR , "gtdbtk-res","gtdbtk.ar53.bac120.summary.clean.tsv"),
