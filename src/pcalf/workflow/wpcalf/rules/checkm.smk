...
rule cm_empty_checkm_tables:
    output:
        taxo   = os.path.join(RESDIR , "checkm-res","tables","checkM_taxonomy.tsv"),
        stats  = os.path.join(RESDIR , "checkm-res","tables","checkM_statistics.tsv"),
        fstats = os.path.join(RESDIR , "checkm-res","tables","checkM_statistics_full.tsv"),
    params:
        genomes = INPUT
    run:
        import pandas as pd
        pd.DataFrame(
            ...
            ).to_csv(str(output.taxo),sep="\t",header=True)
        pd.DataFrame(
            columns = STASCOLS,
            index = list(params.genomes.keys())
            ).to_csv(str(output.stats),sep="\t",header=True)
        pd.DataFrame(
            columns = FULLSTATSCOLS,
            index = list(params.genomes.keys())
            ).to_csv(str(output.fstats),sep="\t",header=True)


rule cm_target_quick_checkm:
    output:
        touch(os.path.join(RESDIR,"checkm-res","checkm.quick.done"))
    input:
...
rule cm_target_checkm:
    output:
        touch(os.path.join(RESDIR,"checkm-res","checkm.done"))
    input:
